...
def create_plot(ood_dataset, roi_id, roi_ood, evaluation, evaluation_ood):
    ...
    roi_name_ood = f"%R2_{roi_ood}" if "rsa" in evaluation else f"R_{roi_ood}"
    ...
    brain_similarity = brain_similarity[['Model', roi_name_id]]
    brain_similarity_ood = brain_similarity_ood[['Model', roi_name_ood]]
    ...
    print(df.columns)
    X = df[[roi_name_id, roi_name_ood]].values
    ...
    x_surf, y_surf = np.meshgrid(
        np.linspace(df[roi_name_id].min(), df[roi_name_id].max(), 10),
        np.linspace(df[roi_name_ood].min(), df[roi_name_ood].max(), 10)
    )
    z_surf = a * x_surf + b * y_surf + c

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(df[roi_name_id], df[roi_name_ood], Z, c=df['color'], s=50)
    ...
    plt.title(f"{roi_name_id} {roi_name_ood}")
    os.makedirs(f"../plots/3d",exist_ok=True)
    plt.savefig(f"../plots/3d/{ood_dataset} {evaluation} {evaluation_ood} {roi_name_id} {roi_name_ood}")
...
if __name__ == '__main__':
    create_plot("imagenet-r","V1", "V4","rsa_natural", "rsa_synthetic")
