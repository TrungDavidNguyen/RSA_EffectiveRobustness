...
def RSA(model_rdms_path, brain_rdms_path, model_name, roi_name, dataset, num_subj):
    ...
    for i in range(num_subj):
        ...
        os.makedirs(f"{dataset}/{roi_name}/rsa_{roi_name}_subj{i+1}", exist_ok=True)
        csv_filename = f"{dataset}/{roi_name}/rsa_{roi_name}_subj{i+1}/{model_name}_RSA.csv"
        all_rois_df.to_csv(csv_filename, index=False)

        all_rois_df = all_rois_df.loc[[all_rois_df['%R2'].idxmax()]]
        csv_filename = f'{dataset}/{roi_name}/rsa_{roi_name}_subj{i+1}/results-rsa-{roi_name}.csv'
    ...
    for layer_dict in all_dicts[num_subj]:
        layer_df = pd.DataFrame.from_dict(layer_dict)
        all_rois_df = pd.concat([all_rois_df, layer_df], ignore_index=True)
    os.makedirs(f"{dataset}/{roi_name}/rsa_{roi_name}_mean", exist_ok=True)
    csv_filename = f"{dataset}/{roi_name}/rsa_{roi_name}_mean/{model_name}_RSA.csv"
    all_rois_df.to_csv(csv_filename, index=False)
    ...
    all_rois_df = all_rois_df.loc[[all_rois_df['%R2'].idxmax()]]
    csv_filename = f'{dataset}/{roi_name}/rsa_{roi_name}_mean/results-rsa-{roi_name}.csv'
    file_exists = os.path.isfile(csv_filename)
    all_rois_df.to_csv(csv_filename, mode='a', index=False, header=not file_exists)
